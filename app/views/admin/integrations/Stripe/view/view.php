...
$query = new Query;
$stripe = $query->load('stripe');
$f3->set('stripe', $stripe[0]);
...
if (isset($_GET['update']) && $_SERVER['REQUEST_METHOD'] === "POST") {
    ...
    if (!empty($schema)) {
        ...
        try {
            ...
            if ($site->dry()) {
                $response->json('error', 'Table was not found in our system.');
                exit;
            }
            ...
            if ($site->save()) {
                $response->json('success', 'Stripe was successfully updated.');
            } else {
